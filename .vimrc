...
"     statusline+=\ BIG\ File
"
"     This cmd originally had
"           \|   echomsg "NOTICE: Not using big file mode."
"           \|   echomsg ""
"     as well, but that feels unnecessary and annoying
"     when you open files.
...
autocmd FileType cpp,c,java,python      set nowrap
...
command! -complete=file -nargs=+ Same call s:RunShellCommandInSame(<q-args>)
function! s:RunShellCommandInSame(cmdline)
    above new
    call s:RunShellCommand(a:cmdline)
    wincmd w
    q
endfunction
...
command! -nargs=+ Findr call s:FindFiles(<q-args>, 1, 0)
command! -nargs=+ Find call s:FindFiles(<q-args>, 0, 0)
command! -nargs=+ Openr call s:FindFiles(<q-args>, 1, 1)
command! -nargs=+ Open call s:FindFiles(<q-args>, 0, 1)
function! s:FindFiles(fileSearchString, useRegexp, doSplit)
    if a:doSplit
        sp
    endif
    if a:useRegexp
        call s:RunShellCommandInSame('find . -iregex '.a:fileSearchString.' | grep -v "\.svn"')
    else
        call s:RunShellCommandInSame('find . -iname '.a:fileSearchString)
    endif
...
nmap <F12> "zdiw:exe 'HeaderguardAddName '.expand(@z)<CR>
nmap <F9> ebi /* <Esc>ea */<Esc>
